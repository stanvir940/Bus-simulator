...
vec3 triSample(vec3 p, vec3 n) {
    n = abs(normalize(n));
    vec3 cx = texture(uTex, p.yz * uTexScale).rgb;
    vec3 cy = texture(uTex, p.xz * uTexScale).rgb;
    vec3 cz = texture(uTex, p.xy * uTexScale).rgb;
    float w = n.x + n.y + n.z + 1e-4;
    return (cx * n.x + cy * n.y + cz * n.z) / w;
}
...
void main() {
    ...
    vec3 albedo = triSample(vWorldPos, N) * uTint;
